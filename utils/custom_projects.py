...
def save_project(name: str, sheet_url: str, columns: dict, gid: int | None = None) -> None:
    ...
    for key, _label, _req in FIELDS:
        ...
        if val and val != "—":
            clean[key] = val
    data[name] = {"sheet_url": url, "gid": int(gid or 0), "columns": clean}
    _save(data)
...
def apply_user_columns(df, columns: dict | None):
    """Rename this file's headers to the standard names. No-op if mapping is empty."""
    if df is None or getattr(df, "empty", True) or not columns:
        return df
    work = df.copy()
    ...
    rename = {}
    for key, user_name in columns.items():
        dest = CANON.get(key)
